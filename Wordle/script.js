  ...
  if (guanyar == false) {
    if (paraulaProbada[intents].length < 5) {
      document.getElementById(
        intents * 5 + paraulaProbada[intents].length + 1
      ).innerHTML = lletra;
  ...
  if (intents < 6) {
    if (guanyar == false) {
      if (paraulaProbada[intents] != 0) {
        document.getElementById(
          intents * 5 + paraulaProbada[intents].length
        ).innerHTML = "";
...
function enviar() {
  if (paraulaProbada[intents].length < 5) {
    ...
    if (paraulaProbada[intents].join("") == paraula) {
      ...
      if (
        paraulaProbada[5].length &&
        paraulaProbada[intents].join("") != paraula
      ) {
        printarColors();
...
function printarColors() {
  ...
  for (let i = 0; i < paraulaProbada[intents].length; i++) {
    ...
    if (paraulaProbada[intents][i] === paraula[i]) {
      document.getElementById(intents * 5 + i + 1).style.backgroundColor =
        "green";
      dictLetras[paraula[i]] -= 1;
    }
  }
